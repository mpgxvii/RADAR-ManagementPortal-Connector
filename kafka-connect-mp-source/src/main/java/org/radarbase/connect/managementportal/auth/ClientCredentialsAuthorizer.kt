...
package org.radarbase.connect.managementportal.auth

import okhttp3.*
import org.radarbase.connect.managementportal.ManagementPortalSourceConnectorConfig
import org.radarbase.connect.managementportal.api.OauthToken
    ...
    private fun requestAccessToken(): OauthToken {
        ...
        return httpClient.newCall(request).execute().use { response ->
            if (response.isSuccessful) {
                ...
                try {
                    ManagementPortalSourceConnectorConfig.mapper.readValue(response.body?.charStream(), OauthToken::class.java)
